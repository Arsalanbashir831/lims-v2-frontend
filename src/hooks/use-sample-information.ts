...
export function useSampleInformationWithoutDetails(page: number = 1, searchQuery?: string, enabled: boolean = true) {
  return useQuery({
    queryKey: searchQuery ? [...SAMPLE_INFORMATION_QUERY_KEYS.search(searchQuery, page), 'without-details'] : [...SAMPLE_INFORMATION_QUERY_KEYS.list(page, searchQuery), 'without-details'],
    queryFn: async () => {
      const jobsData = await sampleInformationService.search(searchQuery?.trim() ?? "", page)
      ...
      if (jobsData.results) {
        for (const job of jobsData.results) {
          try {
            ...
            const sampleLotsResponse = await sampleLotService.getByJobDocumentId(String(job.id))
...
export function useSampleInformationWithDetails(page: number = 1, searchQuery?: string, enabled: boolean = true) {
  return useQuery({
    queryKey: searchQuery ? [...SAMPLE_INFORMATION_QUERY_KEYS.search(searchQuery, page), 'with-details'] : [...SAMPLE_INFORMATION_QUERY_KEYS.list(page, searchQuery), 'with-details'],
    queryFn: async () => {
      const jobsData = await sampleInformationService.search(searchQuery?.trim() ?? "", page)
