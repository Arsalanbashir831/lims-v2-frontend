...
import {
  useSamplePreparations,
  useDeleteSamplePreparation,
} from "@/hooks/use-sample-preparations";

export default function SamplePreparationPage() {
  const [currentPage, setCurrentPage] = useState(1);
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedSamplePrepId, setSelectedSamplePrepId] = useState<
    string | null
  >(null);
  const [isDrawerOpen, setIsDrawerOpen] = useState(false);

  // Use the new hooks
  const { data, error, isFetching } = useSamplePreparations(
    currentPage,
    searchQuery
  );
  ...
  const handleRowClick = useCallback(
    (row: { original: SamplePreparationResponse }) => {
    ...
    () => [
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="font-medium truncate max-w-[28ch]">
              {firstSampleLot?.project_name || "N/A"}
            </div>
          );
        },
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="font-medium truncate max-w-[28ch]">
              {firstSampleLot?.client_name || "N/A"}
            </div>
          );
        },
      },
      {
        ...
        cell: ({ row }) => (
          <span className="font-mono text-sm">{row.original.request_no}</span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-center">{row.original.sample_lots_count}</span>
        ),
      },
      {
        ...
        id: "specimenIds",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Specimen IDs" />
        ),
        cell: ({ row }) => {
          // Get all specimens from all sample lots
          const allSpecimens =
            row.original.sample_lots?.flatMap((lot) => lot.specimens || []) ||
            [];
          ...
          return (
            <span className="text-sm">
              {specimenIds.slice(0, 3).join(", ")}... (+{specimenIds.length - 3}{" "}
              more)
            </span>
          );
        },
  ...
  const toolbar = useCallback(
    (table: TanstackTable<SamplePreparationResponse>) => {
      return (
        <div className="flex flex-col md:flex-row items-center gap-10 w-full">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            ...
            <Button asChild size="sm">
              <Link href={ROUTES.APP.SAMPLE_PREPARATION.NEW}>
                New Preparation
              </Link>
            </Button>
          </div>
    ...
    [handleAdvancedSearch, isFetching]
  );

  const footer = useCallback(
    (_table: TanstackTable<SamplePreparationResponse>) => (
      <ServerPagination
