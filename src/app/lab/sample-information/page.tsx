...
} from "@/hooks/use-sample-information";
...
export default function SampleInformationPage() {
  ...
  const items = (sampleData?.data ?? sampleData?.results ?? []) as Record<
    string,
    unknown
  >[];
  ...
  const hasNext =
    sampleData?.next !== undefined
      ? Boolean(sampleData?.next)
      : totalCount > currentPage * pageSize;
  const hasPrevious =
    sampleData?.previous !== undefined
      ? Boolean(sampleData?.previous)
      : currentPage > 1;
  ...
  const columns: ColumnDef<Record<string, unknown>>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="font-medium truncate max-w-[28ch]">
            {String(row.original.client_name ?? "")}
          </div>
        ),
      },
      {
        ...
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="End User" />
        ),
        cell: ({ row }) => {
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Sample Count" />
        ),
        cell: ({ row }) => (
          <span className="text-center">
            {String(row.original.sample_lots_count ?? 0)}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-right space-x-2 inline-flex">
            <Button variant="secondary" size="sm" asChild>
              <Link
                href={ROUTES.APP.SAMPLE_INFORMATION.EDIT(
        ...
        <div className="flex flex-col md:flex-row items-center gap-10 w-full">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            ...
            <Button asChild size="sm">
              <Link href={ROUTES.APP.SAMPLE_INFORMATION.NEW}>
  ...
  return (
    <>
      <DataTable
        ...
        empty={
          <span className="text-muted-foreground">
            No sample information yet
          </span>
        }
        pageSize={20}
        tableKey="sample-information"
        onRowClick={(row) => {
          setSelectedJobId(String(row.original.id));
